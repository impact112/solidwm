 ...
 *
 * The event handlers of solidwm are organized in an array which is accessed
...
static char statustext[1024] = "solidwm"VERSION;
...
clientmessage(XEvent *e)
{
	...
	if (showsystray && systray && cme->window == systray->win && cme->message_type == netatom[NetSystemTrayOP]) {
		...
		if (cme->data.l[1] == SYSTEM_TRAY_REQUEST_DOCK) {
			...
			XClassHint ch = {"solidwmsystray", "solidwmsystray"};
...
setup(void)
{
	...
	bh = bar_height ? bar_height + vertpadbar: drw->fonts->h + vertpadbar;
	updategeom();
	/* init atoms */
	...
	XChangeProperty(dpy, wmcheckwin, netatom[NetWMName], utf8string, 8,
		PropModeReplace, (unsigned char *) "solidwm", 7);
...
xerror(Display *dpy, XErrorEvent *ee)
{
	...
	|| (ee->request_code == X_CopyArea && ee->error_code == BadDrawable))
		return 0;
	fprintf(stderr, "solidwm: fatal error: request code=%d, error code=%d\n",
...
{
	die("solidwm: another window manager is already running");
	return -1;
}
...
main(int argc, char *argv[])
{
	...
	if (!(dpy = XOpenDisplay(NULL)))
		die("solidwm: cannot open display");
	if (!(xcon = XGetXCBConnection(dpy)))
		die("solidwm: cannot get xcb connection\n");
	checkotherwm();
	setuphook();
	XrmInitialize();
	loadxrdb();
